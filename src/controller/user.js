...
import { randString, createRandomString } from "../common/helper.js";
...
const getUsers = async(req,res) => {
    try {
        let users = await userModel.find({role:'User'},{firstName:1, lastName:1, email:1, phone:1, createdAt:1, status:1, _id:0});
        res.status(200).send({
...
const accountActivation = async(firstName,lastName,email,random_string) => {
            ...
            const mailOptions = {
                ...
                <p>Copy and Paste the token to activate your account</p>
                <p>${random_string}</p>
                `
            }
...
const activateNewAccount = async(req,res)=>{
    try {
        ...
        if(!user){
            ...
            else if(token === user.activationLink){
                user.accountActiveStatus = true;
                user.activationLink="";
                await user.save();
                res.status(200).send({
                    message:"Your account is activated!! Please Log In to continue"
                })
            }
            else{
                res.status(404).send({
